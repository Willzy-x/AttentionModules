...
class DoubleAttention3d(nn.Module):
    ...
    def __init__(self, inchannel, outchannel, reduced_dim):
        ...
        ''' First Attention Step: Feature Gathering '''
        self.dimReduction = nn.Conv3d(inchannel, reduced_dim, kernel_size=1, stride=1)
        self.attention1 = nn.Conv3d(inchannel, inchannel, kernel_size=1, stride=1)
        ''' Second Attention Step: Feature Distribution '''
        self.attention2 = nn.Conv3d(inchannel, inchannel, kernel_size=1, stride=1)
        self.dimExtension = nn.Conv3d(reduced_dim, outchannel, kernel_size=1, stride=1)
    ...
    def forward(self, x):
        # d, c_input, h, w = x.size()
        d, c_input, h, w, l = x.size()

        # dimReduction
        tmpA = self.dimReduction(x)
        _, c_red, _, _, _ = tmpA.size()
        tmpB = self.attention1(x)

        # A.shape: m x dhw
        # B.shape: n x dhw -> transpose: dhw x n
        tmpA = (tmpA.permute(1, 0, 2, 3, 4)).contiguous().view(c_red, d*h*w*l)
        tmpB = (tmpB.permute(1, 0, 2, 3, 4)).contiguous().view(c_input, d*h*w*l)
        tmpB = self.softmax(tmpB, dim=0).permute(1, 0)
        ...
        # V.shape: (n x dhw)
        tmpV = self.attention2(x)
        tmpV = (tmpV.permute(1, 0, 2, 3, 4)).contiguous().view(c_input, d*h*w*l)
        tmpV = self.softmax(tmpV, dim=0)
        output = torch.matmul(output1, tmpV)

        # dimExtension
        output = output.contiguous().view(c_red, d, h, w, l).permute(1, 0, 2, 3, 4)
        output = self.dimExtension(output)
